 ...
 * @subpackage Config
 */
class StealThisTracker_Config_Simple implements StealThisTracker_Config_Interface
{
    /**
     ...
     * @throws StealThisTracker_Config_Error_Missing When the config value is required and missing.
    ...
    public function get( $config_name, $needed = true, $default = null )
    {
        if ( !isset( $this->config_values[$config_name] ) )
        {
            if ( $needed )
            {
                throw new StealThisTracker_Config_Error_Missing( "Value not found: $config_name" );
            }
            return $default;
        }
     ...
     * @param boolean $needed If a config key is missing and $needed is true, this MUST throw StealThisTracker_Config_Error_Missing.
     * @param mixed $default If a config key is missing and $needed is false, this MUST return the according $default.
     * @throws StealThisTracker_Config_Error_Missing When a config value is required and missing.
