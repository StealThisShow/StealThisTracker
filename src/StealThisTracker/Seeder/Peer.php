 ...
 * @subpackage Seeder
 */
class StealThisTracker_Seeder_Peer extends StealThisTracker_Concurrency_Forker
{
    /**
     ...
     *
     * @var StealThisTracker_Config_Interface
     */
    protected $config;
     ...
     *
     * @var StealThisTracker_Persistence_Interface
     */
     ...
     *
     * @var StealThisTracker_Logger_Interface
     */
    protected $logger;
     ...
     */
    public function  __construct( StealThisTracker_Config_Interface $config )
    {
        $this->config       = $config;

        $this->persistence           = $this->config->get( 'persistence' );
        $this->logger                = $this->config->get( 'logger', false, new StealThisTracker_Logger_Blackhole() );
    ...
    public function startParentProcess()
    {
        ...
        if ( $peer_forks < 1 )
        {
            throw new StealThisTracker_Seeder_Error( "Invalid peer fork number: $peer_forks. The minimum fork number is 1." );
        }
    ...
    public function startChildProcess( $slot )
    {
        ...
        if ( $this->persistence instanceof StealThisTracker_Persistence_ResetWhenForking )
        {
     ...
     *
     * @throws StealThisTracker_Seeder_Error_Socket When error happens during creating, binding or listening.
     */
    protected function startListening()
    {
        if ( false === ( $socket = socket_create( AF_INET, SOCK_STREAM, SOL_TCP ) ) )
        {
            throw new StealThisTracker_Seeder_Error_Socket( 'Failed to create socket: ' . socket_strerror( $socket ) );
        }
        ...
        {
            throw new StealThisTracker_Seeder_Error_Socket( 'Failed to bind socket: ' . socket_strerror( $result ) );
        }

        // We set backlog to 5 (ie. 5 connections can be queued) - to be adjusted.
        if ( false === ( $result = socket_listen( $this->listening_socket, 5 ) ) )
        {
            throw new StealThisTracker_Seeder_Error_Socket( 'Failed to listen to socket: ' . socket_strerror( $result ) );
        }
    }
    ...
    protected function communicationLoop()
    {
        ...
        do
        {
            $client = new StealThisTracker_Seeder_Client( $this->listening_socket );
            do
            {
                try
                {
                ...
                catch ( StealThisTracker_Seeder_Error_CloseConnection $e )
                {
     ...
     *
     * @throws StealThisTracker_Seeder_Error_CloseConnection In case when the reqeust is invalid or we don't want or cannot serve the requested torrent.
     * @param StealThisTracker_Seeder_Client $client
     */
    protected function shakeHand( StealThisTracker_Seeder_Client $client )
    {
        ...
        if ( ( $protocol = $client->socketRead( $protocol_length ) ) !== self::PROTOCOL_STRING )
        {
            ...
            throw new StealThisTracker_Seeder_Error_CloseConnection( 'Unsupported protocol.' );
        }
        ...
        if ( !isset( $torrent ) )
        {
            throw new StealThisTracker_Seeder_Error_CloseConnection( 'Unknown info hash.' );
        }
        ...
        if ( 0 < ( $seeders_stop_seeding = $this->config->get( 'seeders_stop_seeding', false, 0 ) ) )
        {
            ...
            if ( $stats['complete'] >= $seeders_stop_seeding )
            {
                ...
                throw new StealThisTracker_Seeder_Error_CloseConnection( 'Stop seeding, we have others to seed.' );
            }
        }
     ...
     *
     * @throws StealThisTracker_Seeder_Error_CloseConnection In case of protocol violation.
     * @param StealThisTracker_Seeder_Client $client
     */
    protected function answer( StealThisTracker_Seeder_Client $client )
    {
        ...
        switch ( $message_type )
        {
            ...
            case 8:
                ...
                break;
            default:
                throw new StealThisTracker_Seeder_Error_CloseConnection( 'Protocol violation, unsupported message.' );
        }
    }
     ...
     */
    protected function sendBlock( StealThisTracker_Seeder_Client $client, $piece_index, $block_begin, $length )
    {
        ...
        // Saving statistics.
        $client->addStatBytes( $length, StealThisTracker_Seeder_Client::STAT_DATA_SENT );
    }
     ...
     * @param StealThisTracker_Seeder_Client $client
     */
    protected function sendBitField( StealThisTracker_Seeder_Client $client )
    {
