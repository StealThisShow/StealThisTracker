 ...
 * @subpackage Seeder
 */
class StealThisTracker_Seeder_Client
{
    /**
     ...
     *
     * @throws StealThisTracker_Seeder_Error_Socket If the accepting is unsuccessful.
    ...
    public function socketAccept( $listening_scket )
    {
        if ( false === ( $this->communication_socket = socket_accept( $listening_scket ) ) )
        {
            ...
            throw new StealThisTracker_Seeder_Error_Socket( 'Socket accept failed: ' . socket_strerror( socket_last_error( $this->listening_socket ) ) );
        }
     ...
     * @throws StealThisTracker_Seeder_Error_Socket If reading fails.
     * @throws StealThisTracker_Seeder_Error_CloseConnection If client closes the connection.
     * @return string
     */
    public function socketRead( $wanted_length )
    {
        ...
        while ( strlen( $message ) < $wanted_length )
        {
            ...
            {
                throw new StealThisTracker_Seeder_Error_Socket( 'Socket reading failed: ' . socket_strerror( $err_no = socket_last_error( $this->communication_socket ) ) . " ($err_no)" );
            }
            if ( '' == $buffer )
            {
                throw new StealThisTracker_Seeder_Error_CloseConnection( 'Client closed the connection.' );
            }
            $message .= $buffer;
        }
