 ...
 */
class StealThisTracker_Persistence_Mysql implements StealThisTracker_Persistence_Interface, StealThisTracker_Persistence_ResetWhenForking
{
    /**
     ...
     *
     * @param StealThisTracker_Torrent $torrent
     */
    public function saveTorrent( StealThisTracker_Torrent $torrent )
    {
        $sql = <<<SQL
INSERT INTO
...
SQL;
    ...
    /**
     * Given a 20 bytes info hash, return an intialized StealThisTracker_Torrent object.
     *
     * Must return null if the info hash is not found.
     *
     * @param string $info_hash
     * @return StealThisTracker_Torrent
     */
...
SQL;
        ...
        if ( 1 == count( $results ) )
        {
            return new StealThisTracker_Torrent(
                new StealThisTracker_File_File( $results[0]['path'] ),
...
SQL;
     ...
     * @throws StealThisTracker_Persistence_Error In case of DB error.
     * @return StealThisTracker_Persistence_MysqlResult Could also return true if there is not resultset.
     */
    protected function query( $query_string, array $parameters )
    {
        ...
        {
            throw new StealThisTracker_Persistence_Error( "Error while performing mysql query.\n" . $this->connection->error . "\n" . $prepared_query );
        }
        ...
        return new StealThisTracker_Persistence_MysqlResult( $results );
    }
    ...
    protected function lazyConnect()
    {
        ...
        {
            throw new StealThisTracker_Persistence_Error( 'Unable to connect to mysql database: ' . mysqli_connect_error() );
        }

        if ( false === $this->connection->select_db( $database ) )
        {
            throw new StealThisTracker_Persistence_Error( "Unable to select database: $database.\n" . $this->connection->error );
        }
    }
     ...
     *
     * @see StealThisTracker_Persistence_ResetWhenForking
     */
