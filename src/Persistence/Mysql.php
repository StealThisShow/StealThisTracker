...
/**
 * Persitence class implementation using MySQL database server.
 *
 * @package StealThisTracker
 * @subpackage Persistence
 */
class Mysql implements PersistenceInterface, ResetWhenForking
{
    /**
...
SQL;
     ...
     * @throws Error In case of DB error.
     * @return MysqlResult Could also return true if there is not resultset.
     */
    protected function query( $query_string, array $parameters )
    {
        ...
        {
            throw new Error( "Error while performing mysql query.\n" . $this->connection->error . "\n" . $prepared_query );
        }
    ...
    protected function lazyConnect()
    {
        ...
        {
            throw new Error( 'Unable to connect to mysql database: ' . mysqli_connect_error() );
        }

        if ( false === $this->connection->select_db( $database ) )
        {
            throw new Error( "Unable to select database: $database.\n" . $this->connection->error );
        }
    }
