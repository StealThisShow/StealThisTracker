...
class Parser
{
    ...
    public function parse()
    {
        ...
        while ( $this->pointer < $string_length )
        {
            ...
            {
                throw new Error\Parse( "Bencode parse error at pointer {$this->pointer}. Unstructured values following each other. Use list/dictionary!", $this->pointer );
            }
            ...
            switch ( $this->string[$this->pointer] )
            {
                ...
                case 'e':
                    ...
                    {
                        throw new Error\Parse( "Bencode parse error at pointer {$this->pointer}. Unexpected endng.", $this->pointer );
                    }
                    if ( isset( $possible_key ) )
                    {
                        ...
                        throw new Error\Parse( "Bencode parse error at pointer {$this->pointer}. Incomplete dictionary.", $this->pointer );
                    }
                    ...
                    break;
                default:
                    throw new Error\Parse( "Bencode parse error at pointer {$this->pointer}. Invalid value.", $this->pointer );
            }
        ...
        if ( 0 != count( $this->container_stack ) )
        {
            throw new Error\Parse( "Bencode parse error at pointer {$this->pointer}. Unclosed dictionary/list", $this->pointer );
        }
    ...
    protected function parseValueInteger()
    {
        ...
        {
            throw new Error\Parse( "Bencode parse error at pointer {$this->pointer}. Missing ending in integer.", $this->pointer );
        }

        $value = new Value\Integer( substr( $this->string, ( $this->pointer + 1 ), ( $end_pointer - $this->pointer - 1 ) ) );
    ...
    protected function parseValueString()
    {
        ...
        {
            throw new Error\Parse( "Bencode parse error at pointer {$this->pointer}. Missing colon in string.", $this->pointer );
        }
