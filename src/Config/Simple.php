 ...
 * @subpackage Config
 */
class Simple implements ConfigInterface
{
    /**
    ...
    public function get( $config_name, $needed = true, $default = null )
    {
        if ( !isset( $this->config_values[$config_name] ) )
        {
            if ( $needed )
            {
                throw new Error\Missing( "Value not found: $config_name" );
            }
            return $default;
        }
     ...
     * @param mixed $defaults If a config key is missing and $needed is false, this MUST return the according $default.
     * @throws Error\Missing When a config value is required and missing.
